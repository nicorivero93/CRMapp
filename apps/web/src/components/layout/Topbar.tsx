  ...
  return (
    <header className="flex h-14 items-center justify-between border-b border-border bg-bg-soft/60 px-4 backdrop-blur">
      <div className="relative max-w-md flex-1">
        ...
        <input placeholder="Buscar contactos, deals, eventos…" className="input pl-9" />
      </div>
      <div className="flex items-center gap-3">
        <div className="text-right text-xs">
          <div className="font-medium">{profile?.name}</div>
          <div className="text-text-faint">{profile?.email}</div>
        </div>
        <div className="grid h-8 w-8 place-items-center rounded-full bg-brand-500/20 text-xs font-semibold text-brand-400">
          {initials}
        </div>
        <button onClick={logout} className="btn-ghost" title="Salir"><LogOut size={15} /></button>
      </div>
    </header>
  );
}
